/*
Para analisar o estado das tabelas em relação a necessidade de vacuum
*/
...
SELECT t.relname                                                                                   "Tabela"
     ...
     , CASE t.n_live_tup
           WHEN 0 THEN '0 bytes'
           ELSE pg_size_pretty((pg_relation_size(t.relid)::NUMERIC * t.n_dead_tup::NUMERIC) /
                               (t.n_live_tup + t.n_dead_tup)::NUMERIC) END                         "Dead Size"
     , CASE t.n_live_tup
           WHEN 0 THEN 0
           ELSE trunc(t.n_dead_tup * 100::NUMERIC / (t.n_live_tup + t.n_dead_tup)::NUMERIC, 1) END "D%"
...
ORDER BY CASE n_live_tup
             WHEN 0 then 0
             ELSE (pg_relation_size(t.relid)::NUMERIC * t.n_dead_tup::NUMERIC) /
                  (t.n_live_tup + t.n_dead_tup)::NUMERIC END
        DESC, relname;
